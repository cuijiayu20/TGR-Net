...
class OurHead(YOLOv8Head):
    ...
    def load_att_embeddings(self, att_embeddings):
        ...
        if self.prev_distribution is not None:
            # todo this
            prev_atts_num = len(torch.load(self.prev_distribution, map_location=self._map_location())['positive_distributions'][self.thrs.index(self.thr)])
        else:
            prev_atts_num = 0
    ...
    def get_all_dis_sim(self, positive_dis, negative_dis):
        ...
        for i in range(len(positive_dis)):
            ...
            positive = positive / positive.sum().clamp_min(1)
            negative = negative / negative.sum().clamp_min(1)
            dis_sim.append(self.get_sim(positive, negative))
        # (num_attributes,)
        return torch.stack(dis_sim).to(self.att_embeddings.device)
    ...
    def combine_distributions(self):
        ...
        prev_distributions = torch.load(self.prev_distribution, map_location=self._map_location())
    ...
    def select_att(self, per_class=25):
        ...
        # print('save current to {}'.format(self.distributions))
        ...
        # distributions = torch.load(self.distributions, map_location=self._map_location())
        ...
        for _ in range(per_class * self.num_classes):
            if len(selected_indices) == 0:
                ...
                idx = unselected_indices[int(score.argmin().item())]
    ...
    def log_distribution(self, att_scores, assigned_scores):
        ...
        for idx, thr in enumerate(self.thrs):
            ...
            num_bins = self.positive_distributions[idx].shape[-1]
            self.positive_distributions[idx].add_(
                self._histogram_attribute_scores(positive_scores, num_bins))
            self.negative_distributions[idx].add_(
                self._histogram_attribute_scores(negative_scores, num_bins))

    @staticmethod
    def _histogram_attribute_scores(scores, num_bins):
        num_attributes = scores.shape[-1]
        if scores.shape[0] == 0:
            return scores.new_zeros((num_attributes, num_bins))
        bin_indices = (
            scores.clamp(0, 1) * num_bins).long().clamp_max(num_bins - 1)
        offsets = (
            torch.arange(num_attributes, device=scores.device) * num_bins)
        flattened = (bin_indices + offsets[None]).reshape(-1)
        return torch.bincount(
            flattened,
            minlength=num_attributes * num_bins,
        ).reshape(num_attributes, num_bins).to(scores.dtype)
